            ...
            <nav class="navbar navbar-expand-lg navbar-light bg-white py-lg-0 px-lg-3">
                ...
                <div class="collapse navbar-collapse" id="navbarCollapse">
                    <div class="navbar-nav">
                        ...
                        <a href="message.php" class="nav-item nav-link">Chat <?php
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_medical_history'])) {
    ...
    $one = isset($_POST['one']) ? htmlspecialchars($_POST['one']) : '';
    $two = isset($_POST['two']) ? htmlspecialchars($_POST['two']) : '';
    $two_answer = isset($_POST['two_answer']) ? htmlspecialchars($_POST['two_answer']) : '';
    $three = isset($_POST['three']) ? htmlspecialchars($_POST['three']) : '';
    $three_answer = isset($_POST['three_answer']) ? htmlspecialchars($_POST['three_answer']) : '';
    $four = isset($_POST['four']) ? htmlspecialchars($_POST['four']) : '';
    $four_answer = isset($_POST['four_answer']) ? htmlspecialchars($_POST['four_answer']) : '';
    $five = isset($_POST['five']) ? htmlspecialchars($_POST['five']) : '';
    $five_answer = isset($_POST['five_answer']) ? htmlspecialchars($_POST['five_answer']) : '';
    $six = isset($_POST['six']) ? htmlspecialchars($_POST['six']) : '';
    $seven = isset($_POST['seven']) ? htmlspecialchars($_POST['seven']) : '';
    $allergies_str = isset($_POST['allergies_str']) ? htmlspecialchars($_POST['allergies_str']) : '';
    $pregnant = isset($_POST['pregnant']) ? htmlspecialchars($_POST['pregnant']) : '';
    $nursing = isset($_POST['nursing']) ? htmlspecialchars($_POST['nursing']) : '';
    $pills = isset($_POST['pills']) ? htmlspecialchars($_POST['pills']) : '';
    $blood_type = isset($_POST['blood_type']) ? htmlspecialchars($_POST['blood_type']) : '';
    $conditions_str = isset($_POST['conditions_str']) ? htmlspecialchars($_POST['conditions_str']) : '';
    ...
    echo "One: $one\n";
    echo "Two: $two\n";
    echo "Two Answer: $two_answer\n";
    echo "Three: $three\n";
    echo "Three Answer: $three_answer\n";
    echo "Four: $four\n";
    echo "Four Answer: $four_answer\n";
    echo "Five: $five\n";
    echo "Five Answer: $five_answer\n";
    echo "Six: $six\n";
    echo "Seven: $seven\n";
    echo "Allergies: $allergies_str\n";
    echo "Pregnant: $pregnant\n";
    echo "Nursing: $nursing\n";
    echo "Pills: $pills\n";
    echo "Blood Type: $blood_type\n";
    ...
    if (!empty($med_id) && !empty($med_prev_dent) && !empty($med_last_vi) && !empty($med_date)) {
        ...
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ssssssssssssssssssss", 
                ...
                $blood_type, 
                $conditions_str,
                $med_id // This should be the last parameter
            );

            // Execute the statement
            if ($stmt->execute()) {
                // Success: provide feedback to the user (or redirect)
                echo "<script>alert('Record updated successfully.'); window.location.href='medical-history.php';</script>";
            } else {
